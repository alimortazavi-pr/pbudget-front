import type { MessageTree } from "../../types";

export const adminMessages: MessageTree = {
  transactionSaved: "Transaction saved",
  contentLoadFailed: "Failed to load content",
  contentManagement: "Content management",
  csvHint: "Excel / Google Sheets",
  exportSuccess: "Export {{name}} completed successfully",
  restoreSuccess: "Restored {{count}} documents",
  connectedLatency: "Connected ({{ms}}ms)",
  deleteBankConfirm: "Delete bank «{{name}}»?",
  bankParserBluBank: "BluBank (Excel)",
  bankParserPasargad: "Pasargad Bank (Excel)",
  protectedCollectionsHint:
    "backuphistories and adminauditlogs collections are protected",
  subscriptionsTitle: "Subscription management",
  subscriptionsDescription: "Control plans, capabilities, and user assignments.",
  plans: "Plans",
  subscribers: "Subscribers",
  newPlan: "New plan",
  planName: "Plan name",
  planSlug: "Plan slug",
  planDescription: "Plan description",
  planPrice: "Price",
  planPeriod: "Period",
  planFeatures: "Capabilities",
  featureKey: "Feature key",
  featureLabel: "Feature label",
  featureLimit: "Limit (optional)",
  addFeature: "Add capability",
  activePlan: "Active",
  inactivePlan: "Inactive",
  savePlan: "Save plan",
  assignSubscription: "Assign subscription",
  selectUser: "Select a user",
  selectPlan: "Select a plan",
  assign: "Assign",
  revoke: "Revoke subscription",
  noSubscriptions: "No subscriptions yet.",
  subscriptionSaved: "Subscription saved",
  planSaved: "Plan saved",
  planArchived: "Plan archived",
  archivePlanConfirm: "Archive plan «{{name}}»?",
  highlighted: "Featured plan",
  periodMonthly: "Monthly",
  periodYearly: "Yearly",
  periodLifetime: "Lifetime",
  periodCustom: "Custom",
  periodDays: "Number of days",
  contactMessage: "Activation message",
  manualActivationHint: "Manual assignment, no payment gateway",
  editPlan: "Edit plan",
  cancelEdit: "Cancel editing",
  planEditorHint: "Configure this plan's price, period, and access controls.",
  planRequired: "Enter a plan name and slug.",
  priceUnit: "Price unit",
  featureEditorHint: "Enable or disable capabilities and optionally set limits.",
  noFeatures: "No capabilities added yet.",
  featureEnabled: "Enabled",
  userColumn: "User",
  planColumn: "Plan",
  statusColumn: "Status",
  expiresColumn: "Expires",
  changePlan: "Change plan",
};
