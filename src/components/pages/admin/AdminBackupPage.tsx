...
export function AdminBackupPage() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      showToast("بارگذاری اطلاعات بکاپ ناموفق بود", "danger");
    } finally {
      setLoading(false);
    }
  }, []);
  ...
  const runBackup = async () => {
    ...
    try {
      ...
      showToast("بکاپ به تلگرام ارسال شد", "success");
      void load();
    } catch {
      showToast("اجرای بکاپ ناموفق بود", "danger");
    } finally {
      setRunning(false);
    }
  };
  ...
  const downloadLocal = async () => {
    ...
    try {
      ...
      showToast("فایل ZIP دانلود شد", "success");
    } catch {
  ...
  const handleZipImport = async (file: File | undefined) => {
    ...
    try {
      ...
      showToast("بازیابی ZIP ناموفق بود", "danger");
    } finally {
      setRunning(false);
  ...
  return (
    <div className="space-y-6">
      <div>
        <h3 className="text-lg font-bold">بکاپ و خروجی</h3>
      ...
      <section className="grid gap-4 lg:grid-cols-2">
        <div className="glass rounded-2xl p-6">
          <div className="mb-4 flex items-center gap-2">
            ...
            <h4 className="font-bold">بکاپ زمان‌بندی‌شده</h4>
          </div>
          <dl className="space-y-3 text-sm">
            <div className="flex justify-between gap-4">
              <dt className="text-muted">زمان‌بندی</dt>
              <dd>{info?.schedule.description}</dd>
            </div>
            <div className="flex justify-between gap-4">
              <dt className="text-muted">منطقه زمانی</dt>
              <dd>{info?.schedule.timezone}</dd>
            </div>
            <div className="flex justify-between gap-4">
              <dt className="text-muted">تلگرام</dt>
              <dd>{info?.telegram.enabled ? "فعال" : "غیرفعال"}</dd>
            </div>
            <div className="flex justify-between gap-4">
              <dt className="text-muted">حداکثر حجم</dt>
        ...
        <div className="glass rounded-2xl p-6">
          <div className="mb-4 flex items-center gap-2">
            ...
            <h4 className="font-bold">آخرین بکاپ موفق</h4>
          </div>
          {info?.lastRun ? (
            <dl className="space-y-3 text-sm">
              <div className="flex justify-between gap-4">
              ...
              <div className="flex justify-between gap-4">
                <dt className="text-muted">کالکشن‌ها</dt>
                <dd>{toPersianDigits(info.lastRun.collections)}</dd>
              </div>
              <div className="flex justify-between gap-4">
                <dt className="text-muted">اسناد</dt>
            ...
            <p className="text-sm text-muted">هنوز بکاپ موفقی ثبت نشده است.</p>
          )}
        </div>
      </section>

      <section className="glass rounded-2xl p-6">
        <h4 className="mb-4 font-bold">عملیات</h4>
        ...
        <div className="mt-4 flex flex-wrap items-center gap-4">
          <div className="flex flex-wrap items-center gap-2">
            <span className="text-sm text-muted">فرمت خروجی:</span>
          ...
          <div className="flex flex-wrap items-center gap-2">
            <span className="text-sm text-muted">حالت بازیابی:</span>
            {(
              [
      ...
      <section className="glass overflow-hidden rounded-2xl">
        <div className="border-b border-border/50 px-5 py-4">
          <h4 className="font-bold">تاریخچه بکاپ</h4>
        </div>
      ...
      <section className="rounded-2xl border border-border/60 bg-surface-secondary/40 p-5">
        <h4 className="mb-2 font-bold text-sm">راهنمای بازیابی</h4>
